...
int choose_page_to_be_replaced(ssystem* S) {
    ...
    // FIFO: la primera página cargada (listoccupied->next) será reemplazada
    victim_frame = S->frt[S->listoccupied].next;
    int victim = S->frt[victim_frame].page;

    if (S->detailed)
        printf("@ Choosing (FIFO) P%d of F%d to be replaced\n", victim, victim_frame);

    // actualizar la lista circular de ocupados
